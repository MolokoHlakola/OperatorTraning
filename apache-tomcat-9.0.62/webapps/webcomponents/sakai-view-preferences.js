function getViewPreferences(tool, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(url, { cache: "no-store", headers: { "Content-Type": "application/json" } }).then(d => d.json()).then(json => {
      ...
      console.error(error);resolve(null);
    });
  });
}
...
function updateViewPreferences(tool, value, options) {
  ...
  fetch(url, { method: "PUT", cache: "no-store" }).catch(error => console.error(`Failed to update view preferences for tool '${tool}'. Take a look at the server logs?`));
}
