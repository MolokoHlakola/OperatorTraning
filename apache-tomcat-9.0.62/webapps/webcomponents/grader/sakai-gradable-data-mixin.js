...
let gradableDataMixin = Base => class extends Base {
  ...
  loadGradableData(gradableId, courseId, submissionId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fetch(`/direct/assignment/gradable.json?gradableId=${gradableId}`, { cache: "no-cache", credentials: "same-origin" }).then(res => res.json()).then(gradableData => {
        ...
        fetch(`/direct/assignment/grades.json?gradableId=${gradableId}&courseId=${courseId}`, { cache: "no-cache", credentials: "same-origin" }).then(res => res.json()).then(gradesData => {
